...
def depth_loss(y_true, y_pred, theta=0.1, device="cuda", maxDepth=3):
    ...
    l_ssim = torch.clamp((1 - ssim(y_pred, y_true, val_range=maxDepth, full=False)) / 0.5, 0, 1)

    w1 = theta
    w2 = 1
    w3 = 1

    loss = w1 * l1_loss + w2 * torch.mean(l_edges) + w3 * l_ssim
